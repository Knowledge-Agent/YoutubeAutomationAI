...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `${envConfigs.app_url}${locale !== defaultLocale ? `/${locale}` : ''}/blog/category/${slug}`,
    },
  };
}
...
export default async function CategoryBlogPage({
  ...
  if (!categoryData) {
    ...
    currentCategory = localCategories.find((category) => category.slug === slug) || null;
  ...
  const structuredData = createStructuredDataGraph([
    buildCollectionPageStructuredData({
      ...
      title: currentCategory.title || slug,
      description: pageData.sections?.blog?.description,
      posts,
    }),
    buildBreadcrumbStructuredData({
      ...
      items: [
        { name: 'Home', url: '/' },
        { name: 'Blog', url: '/blog' },
        { name: currentCategory.title || slug, url: `/blog/category/${slug}` },
      ],
    }),
  ]);
